                          ...
                          <td className="px-4 py-4">
                            <div className="flex items-center gap-2">
                              <button
                                type="button"
                                onClick={async () => {
                                  const currentVal = member.dashboard_otp_enabled ?? false;
                                  setMemberActionLoading(true);
                                  setMemberActionError("");
                                  try {
                                    await updateDashboardOtpStatus(orgId, member.id, !currentVal);
                                    await reloadMembers();
                                    if (selectedMemberId === member.id) {
                                      await refreshSelectedMember();
                                    }
                                    setFlash("Configuración de OTP de usuario actualizada.");
                                  } catch (presetError) {
                                    setMemberActionError(presetError instanceof Error ? presetError.message : "Error al actualizar OTP.");
                                  } finally {
                                    setMemberActionLoading(false);
                                  }
                                }}
                              ...
                              >
                                <span
                                  className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                                    member.dashboard_otp_enabled ? "translate-x-4" : "translate-x-0"
                                  }`}
                                />
                              </button>
                              <span className={`text-[10px] font-bold uppercase tracking-wider ${
                                member.dashboard_otp_enabled ? "text-emerald-600 dark:text-emerald-400" : "text-dark-6"
                              }`}>
                                {member.dashboard_otp_enabled ? "Requerido" : "Desactivado"}
                              </span>
                            </div>
                          </td>
                          <td className="px-4 py-4">
