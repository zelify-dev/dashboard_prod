...
export default function PaymentsUserTransactionsPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const userIdFromUrl = searchParams.get("userId") ?? "";
  const accountIdFromUrl = searchParams.get("accountId") ?? "";
  const userNameFromUrl = searchParams.get("userName") ?? "";
  const returnTab = searchParams.get("tab") ?? "internal";
  ...
  const [includeInterbank, setIncludeInterbank] = useState(true);

  const filteredItems = useMemo(() => {
    const allowed = new Set<string>();
    if (includeInternal) allowed.add("internal_transfer");
    if (includeInterbank) allowed.add("interbank_transfer");
    return historyItems.filter((it) => (it.reference_type ? allowed.has(it.reference_type) : false));
  }, [historyItems, includeInternal, includeInterbank]);
  ...
  return (
    <div className="mx-auto w-full max-w-[1400px]">
      <Breadcrumb pageName="Movimientos por usuario" />

      <div className="mb-4">
        <button
          onClick={() => router.push(`/pages/products/payments/workflow?tab=${encodeURIComponent(returnTab)}`)}
      ...
      <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card sm:p-7.5">
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-sm text-dark-6 dark:text-dark-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <label className="flex items-center gap-2 rounded-md border border-stroke bg-white px-3 py-1.5 text-sm font-medium text-dark dark:border-dark-3 dark:bg-gray-dark dark:text-white">
              <input type="checkbox" checked={includeInterbank} onChange={(e) => setIncludeInterbank(e.target.checked)} />
              Interbancarias
            </label>
